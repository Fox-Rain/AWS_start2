# 거래소 ->  Binance Upbit Bithumb Huobi Gateio        ...... ... 더 추가...?   Kucoin, OKEX   <-- 추가 고려


# 추가할 것들....   1. 지갑이 여러개 닫혔을 경우 어느것을 매수할 것인지?   2. 타거래소 지갑감지 시스템    3. 거래량이 적은 거래소의 경우 어느 정도의 호가창일때 매수해야 하는지?
# 다시해야할것들..  ---> / 후오비 바이낸스 게이트아이오 시장가매수 오류 /

# 2021.10.7     Gate.io 추가 + /USDT if문 제거로 실행속도 높임
...
upbit_BTC_coinList = ["CELO","LRC","ONIT","FOR","RVN","FIL","DAD","BNT","SNX","LUNA","PSG","NKN","RLC","CRV","HBD","DGB","PROM","LINA","RSR","NMR","ALGO","DNT","INJ","CTSI","SUN","CHR","AUCTION","GO","GXC","UNI","NEAR","JUV","GRT","OXT","OGN","COMP","MKR","SOLVE"]
upbit_BTC_wallet_coinList = ["FX","VAL","PROM","JUV","FOR","ONIT","SNX","NKN","LINA","CTSI","HBD","AUCTION","CELO","INJ","PSG","LRC","DAD","NMR","BNT","SUN","GXC","GRT","DNT","OXT","COMP","CRV","RLC","DAI","MKR"]
...
# huobi coinList     /USDT 만
huobi_coinList = []

for coin in huobi.fetch_tickers():
    if("/USDT" in coin):
        huobi_coinList.append(coin)

# gate io coinList    /USDT 만
gateio_coinList = []

for coin in gate_io.fetch_tickers():
    if("/USDT" in coin):
        gateio_coinList.append(coin)
...
while(1):
    ...
    try:
        ...
        if(notice!=current_notice): # 최근 공지가 새로운 공지일 경우
            ...
            # case 1) 업비트 입출금 공지사항
            if("업비트입니다." in notice and ("입출금" in notice or "입금" in notice)):
                if("(완료)" in notice or "재개합니다"in notice or "중단 시점" in notice):
            ...
            # 바이낸스 상장 공시                      (Ex) Binance will list .... crypto    ---> BUY )      
                ...
                if("List" in notice and (("Delist" in notice)==False)): # 상장공시 일 경우
                    # huobi_Coin
                    for coin in huobi_coinList:
                        
                        coin_symbol = coin[:-5] # USDT를 빼기 심볼 자체만
                        
                        if(("("+str(coin_symbol)+")") in notice):
                    ...
                    # gateio_Coin
                    for coin in gateio_coinList:
                        
                        coin_symbol = coin[:5] # USDT빠진 순수 코인 심볼
                        if("("+str(coin_symbol)+")" in notice):
                            seed = gate_io.fetch_balance()['USDT']["free"]

                            df = gate_io.fetch_ohlcv(coin_symbol+str("/USDT"),"1m")
                            coin_current_price = df[-1][4] # 현재가
                            gate_io.create_market_buy_order(coin_symbol+"/USDT",seed/coin_current_price*0.95)

                            while(1):
                                try:
                                    buy_bot.sendMessage(chat_id=id2,text=str(datetime.datetime.now())+"\n"+"\n"+notice+"\n"+"\n"+"(Gate io) "+str(coin)+" 바이낸스 상장 -> 매수하였습니다.")
                                    break
                                except:
                                    print("re-message")
                                    continue                 
