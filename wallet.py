...
headers = {"User_Agent":"https://developers.whatismybrowser.com/useragents/parse/?analyse-my-user-agent=yes"}
                ...
                for i in range(0,counting):
                    ...
                    if(coin_symbol!="None"): # 트위터가 쓸모있는 트윗일 경우
                        ...
                        if(exchange_counting!=0): # 일단 코인이 존재하는 거래소가 1개 이상일 경우
                            ...
                            if((upbit_flag==False or upbit_rate>2.5) and (upbit_BTC_flag==False or upbit_BTC_rate>2.5) and (bithumb_flag==False or bithumb_rate>2.5) and (binance_flag==False or binance_rate>2.5) and (huobi_flag==False or huobi_rate>2.5)): # 평균적으로 2.3퍼이상의 상승을 보였을 경우 매수를 실행
